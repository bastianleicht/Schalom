...
module.exports = async (client, guild) => {
    try {
        client.db.query('SELECT * FROM server WHERE guildID = ?', [guild.id], (err, { length }) => {
            if(err) return console.error(`MYSQL: query guildID ERROR: ${err}`);
            if(length) {
                //  Guild already exists in DB
                console.log(`${client.user.username} just re-joined a guild! \nGuild Name: '${guild.name}' \nGuild ID: '${guild.id}' \nGuild Membercount: '${guild.memberCount}'!`);
                ...
                try {
                    client.db.query('INSERT INTO server ( guildID ) values (?)', [guild.id], (err, { insertId }) => {
                        ...
                        if (err) return console.error(`MYSQL: guildCreate (id: ${guild.id}) insertion failed! Error: ${err}`);
                        if (insertId === null) return console.error(`MYSQL: guildCreate (id: ${guild.id}) insertion failed!`);

                        console.log(`${client.user.username} just joined a new guild! \nGuild Name: '${guild.name}' \nGuild ID: '${guild.id}' \nGuild Membercount: '${guild.memberCount}'!`);
